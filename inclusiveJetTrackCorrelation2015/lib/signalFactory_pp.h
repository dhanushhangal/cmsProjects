...
//	TFile *wf = TFile::Open(name+"_signal.root","recreate");
	TH1D** signal_deta = new TH1D*[signal->nz-1];
	TH1D** signal_dphi = new TH1D*[signal->nz-1];
	TH1D** signal_dr   = new TH1D*[signal->nz-1];
	TH1D** signal_drGeo= new TH1D*[signal->nz-1];
	...
	for(int i=0; i<signal->nz-1; ++i){
			...
			sig[i]->SetName(stmp);
			geo=jts.drJetShape(sig[i],signal_dr[i]);
	...
	TCanvas * c_pp = new TCanvas(name+"_pp_signal", "", 1000, 2500);
	...
	for(int i=0; i<signal->nz-1; ++i){
			...
			signal_drGeo[i]->Draw("same");
	}
	c_pp->SaveAs(name+"_pp_signal.png");
	//delete [] sig;
	//delete [] signal_deta;
	//delete [] signal_dphi;
	//delete [] signal_dr;
