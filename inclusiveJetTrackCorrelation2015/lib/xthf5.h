...
class xthf5 {
	public:
		...
		xthf5(TString name, TString title, int nxbin, float xmin,float xmax, \
		...
		TH2F* hist (int i, int j, int k); //input the 2D index and return the corresponding 1 D index;
...
xthf5::xthf5(TString name, TString title, int nxbin, float xmin,float xmax, \
		int nybin, float ymin, float ymax, int nzbin, float *zbin1, int nwbin, float *wbin1, int nrbin,
	...
	for(int i=0; i<nz; ++i){
		for(int j=0; j<nw; ++j){
			for(int k=0; k<nr; ++k){
				...
				hf5[i+j*nz+k*nz*nw]= new TH2F(hname+Form("_%d_%d_%d",i,j, k), tmp,
						nxbin, xmin, xmax, nybin, ymin, ymax);
				hf5[i+j*nz+k*nz*nw]->Sumw2();
			}
		}
	}
}

void xthf5::axis_title_setup(TString name, TString *&aname, int nbin, float *bins, float *&bins2){
...
TH2F* xthf5::hist(int i, int j, int k){
	return hf5[i+j*nz+k*nz*nw];
}
int xthf5::Fill(float x,float y,float z, float w, float r,float wit){
...
int xthf5::checkRebin(int n, float *bins, int n2, float*bins2, std::vector <int > & indx){
	indx.clear();
...
int xthf5::Read(TString name, TString title, TFile *f , int nz1, float *zbin1, int nw1, float *wbin1, int nr1, float *rbin1, TString ztitle,TString wtitle,TString rtitle){
	
	if(!nhist) {
		std::cout<<"thf5 is filled by other histograms, load failed!"<<std::endl;
		return 0;
	}
	nz = nz1+1;
	nw = nw1+1;
	...
	for(int i=0; i<nz; ++i){
		for(int j=0; j<nw; ++j){
			for(int k=0; k<nr; ++k){
				nhist++;
				tmp = " :"+zname[i]+wname[j]+rname[k];
				hf5[i+j*nz+k*nz*nw]= (TH2F*)f->Get(hname+Form("_%d_%d_%d",i,j, k));
			}
		}
	}
	return nhist;
